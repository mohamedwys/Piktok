...
const TITLES: Record<Locale, string> = {
  en: 'Child Safety — Mony',
  fr: 'Sécurité des enfants — Mony',
  ar: 'سلامة الأطفال — Mony',
};

async function loadContent(locale: Locale): Promise<string> {
  const file = path.join(process.cwd(), 'src/content/legal', `child-safety.${locale}.md`);
  try {
    return await fs.readFile(file, 'utf-8');
  } catch {
    const fallback = path.join(process.cwd(), 'src/content/legal', 'child-safety.en.md');
