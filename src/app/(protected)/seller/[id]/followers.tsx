...
import { useFollowers } from '@/features/marketplace/hooks/useFollowers';
import {
...
export default function SellerFollowersScreen(): React.ReactElement {
  ...
  const followersQuery = useFollowers(sellerId);

  const items = followersQuery.data?.pages.flatMap((page) => page) ?? [];
  ...
  const headerTitle = seller?.name
    ? t('social.followersOfTitle', { name: seller.name })
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          followersQuery.isLoading ? (
            ...
            <FollowerListEmpty
              ...
              title={t('social.noFollowersTitle')}
            />
          )
        }
        ListFooterComponent={
        ...
        refreshControl={
          <RefreshControl
            refreshing={followersQuery.isRefetching && !followersQuery.isFetchingNextPage}
            onRefresh={() => {
